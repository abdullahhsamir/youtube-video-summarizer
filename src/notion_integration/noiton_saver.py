...
class NotionSaver:
    def __init__(self, notion_token=None, parent_page_id=None):
        ...
        if not self.notion_token:
            raise ValueError(
                "NOTION_TOKEN not found. Please provide it as parameter or set it in .env file"
            )
        ...
        if not self.parent_page_id:
            print(
                "� No parent page ID found. You'll need to set it manually using set_parent_page_id()"
            )
    ...
    def from_env(cls):
        ...
        """
        return cls(notion_token=None, parent_page_id=None)
    ...
    def create_page(self, title, content, youtube_url=None):
        ...
        if youtube_url:
            page_data["children"].append(
                {"object": "block", "type": "embed", "embed": {"url": youtube_url}}
            )
